...
1. Update `.env-local-elements`.
2. Run `sudo docker-compose -f docker-compose-local-element.yml build` if not previously built.
3. Then `sudo docker-compose -f docker-compose-local-element.yml up` to run appliaction.
